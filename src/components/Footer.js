            ...
            <div className="grid grid-cols-20">
              ...
              <div className="col-span-8">
                ...
                {contact.map((item) => (
                  <a key={item.name} href={item.link || "#"} className="flex items-center gap-2 mt-2 text-md text-gray-600 dark:text-gray-400 hover:underline">
                    {item.icon && <FontAwesomeIcon icon={item.icon} className="text-gray-500 dark:text-gray-400" />}
                    <span>{item.name}</span>
                  </a>
                ))}
              </div>
              ...
              <div className="col-span-2">
                ...
                {navigation.slice(0, 3).map((item) => (
                  <a key={item.name} href={item.href || "#"} className="block mt-2 text-md text-gray-600 dark:text-gray-400 hover:underline">
                    {item.name}
                  </a>
                ))}
              </div>
